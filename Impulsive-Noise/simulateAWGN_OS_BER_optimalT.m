...
for idxEbn0 = 1:size(ebn0List, 2)
    ...
    parfor idxRun = 1:(totalSigCount / sigBatchPerLoop)
        ...
        % rxDeepClipFDSig = 1 / (sqrt(fftSize) .* sqrt(OSFactor)) .* fft(rxDeepClipSig, fftSize, 1);
        % rxDemapDeepClipSig = scDemap(rxDeepClipFDSig, fftSize, nullIdx);
        ...
        % rxReplaceFDSig = 1 / (sqrt(fftSize) .* sqrt(OSFactor)) .* fft(rxReplaceSig, fftSize, 1);
        % rxDemapReplaceSig = scDemap(rxReplaceFDSig, fftSize, nullIdx);
        ...
        % rxReplaceClipBlankFDSig = 1 / (sqrt(fftSize) .* sqrt(OSFactor)) .* fft(rxReplaceClipBlankSig, fftSize, 1);
        % rxDemapReplaceClipBlankSig = scDemap(rxReplaceClipBlankFDSig, fftSize, nullIdx);
