...
INprobList = [0.1];  % Probability of impulsive noise occurring
...
allData = dictionary;
dataSize = [length(INprobList) length(ampThresholdList)];

for idxProcess = startProcess:length(processName)
    name = processName{idxProcess};
    switch name
        case 'PGIR'
        ...
        otherwise
            error("Don't have `%s` process. Please check.", name);
    end
...
for idxINprob = 1:length(INprobList)
    ...
    for idxAmpThreshold = 1:length(ampThresholdList)
        ...
        for idxProcess = startProcess:length(processName)
            ...
            allData(name).K_pooled(idxINprob, idxAmpThreshold) = Kp;
            allData(name).K_active(idxINprob, idxAmpThreshold) = Ka;
            allData(name).SNR_pooled(idxINprob, idxAmpThreshold) = SNRp;
            allData(name).SNR_active(idxINprob, idxAmpThreshold) = SNRa;
            allData(name).Beta(idxINprob, idxAmpThreshold) = beta;
...
for idxINprob = 1:length(INprobList)
    ...
    for idxLoop = 1:(baseSigCount(idxINprob) / sigBatchPerLoop)
        ...
        for idxAmpThreshold = 1:length(ampThresholdList)
            ...
            for idxProcess = startProcess:length(processName)
                ...
                Kc = allData(name).K_active(idxINprob, idxAmpThreshold);
        ...
        progress = idxLoop / (baseSigCount(idxINprob) / sigBatchPerLoop);
        fprintf("Now done %.2f%% at %s\n", progress * 100, getTimeStr);
    end

    for idxProcess = startProcess:length(processName)
        name = processName(idxProcess);
        allData(name).BER(idxINprob, :) = allErrBitCount(name).ErrorBitCount / (baseSigCount(idxINprob) * sigConfig.bitsPerOFDMSymbol);
    end
...
%%
res = struct();
res.scheme    = processName(startProcess:end);
...
for idxINprob = 1:length(INprobList)
    ...
    res.p = INprobList(idxINprob);
    
    tbl = optTable(res);
    save(sprintf('optSummary_M%d_p%g.mat', modOrder, INprobList(idxINprob)), 'tbl');
    disp(tbl);
end
...
%     for idxProcess = startProcess:length(processName)
%         name = processName{idxProcess};
%         plot(ampThresholdList, abs(allData(name).K_pooled(idxINprob, :)), '-', Color=c(idxINprob, :), ...
%             DisplayName=sprintf('K^{%s}_pooled', name), LineWidth=linewidth, ...
%             Marker=markerList{idxProcess}, MarkerIndices=markerIdx, MarkerSize=markersize);
%         plot(ampThresholdList, abs(allData(name).K_active(idxINprob, :)), '--', Color=c(idxINprob, :), ...
%             DisplayName=sprintf('K^{%s}_active', name), LineWidth=linewidth, ...
%             Marker=markerList{idxProcess}, MarkerIndices=markerIdx, MarkerSize=markersize);
%     end
%     % plot(ampThresholdList, abs(K_PGIR_pooled(idxINprob, :)), '-', Color=c(idxINprob, :), ...
%     %     DisplayName='K_{pooled}', MarkerIndices=markerIdx, LineWidth=linewidth, MarkerSize=markersize);
%     % plot(ampThresholdList, abs(K_PGIR_active(idxINprob, :)), '--', Color=c(idxINprob, :), ...
%     %     DisplayName='K_{active}', MarkerIndices=markerIdx, LineWidth=linewidth, MarkerSize=markersize);
% end
% 
% nColor = length(INprobList);
% nStyle = 2 + length(processName);
% 
% colorTexts = {"p=0.1", "p=0.01", "p=0.001"};
% % styleTexts = {"K_{pooled}", "K_{active}", "K^{blanking}_{pooled}", "K^{blanking}_{active}"};
% styleTexts = {'K_{pooled}', 'K_{active}', processName{:}};
% styleLines = {'-', '--', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-'};
...
% ylabel('K');
% title(sprintf('N = %d, Oversampling %d, K comparison', fftSize, OSFactor), ...
...
for idxINprob = 1:length(INprobList)
    for idxProcess = startProcess:length(processName)
        name = processName{idxProcess};
        plot(ampThresholdList, 10 * log10(allData(name).SNR_pooled(idxINprob, :)), '-', Color=c(idxINprob, :), ...
            DisplayName=sprintf('SNR^{%s}_pooled', name), LineWidth=linewidth, ...
            Marker=markerList{idxProcess}, MarkerIndices=markerIdx, MarkerSize=markersize);
        plot(ampThresholdList, 10 * log10(allData(name).SNR_active(idxINprob, :)), '--', Color=c(idxINprob, :), ...
...
colorTexts = cellstr('p='+string(INprobList));
...
title(sprintf('N = %d, Oversampling %d, %d QAM, SNR comparison', fftSize, OSFactor, modOrder), ...
    Interpreter="latex", FontSize=16);
...
for idxINprob = 1:length(INprobList)
    for idxProcess = startProcess:length(processName)
        ...
        plot(ampThresholdList, allData(name).Beta(idxINprob, :), styles{idxProcess}, ...
            Color=c(idxINprob, :), DisplayName=sprintf('p=%.3f, %s', INprobList(idxINprob), name), ...
            LineWidth=linewidth, MarkerIndices=markerIdx, MarkerSize=markersize);
    end
...
xlabel('Unknown Data Threshold (T)');
ylabel('\beta');
title(sprintf('N = %d, Oversampling %d, $\\beta$', fftSize, OSFactor), ...
    Interpreter="latex", FontSize=16);
...
for idxINprob = 1:length(INprobList)
    for idxProcess = startProcess:length(processName)
        name = processName{idxProcess};
        plot(ampThresholdList, allData(name).BER(idxINprob, :), styles{idxProcess}, ...
            Color=c(idxINprob, :), DisplayName=sprintf('p=%s, %s', string(INprobList(idxINprob)), name), ...
            LineWidth=linewidth, MarkerIndices=markerIdx, MarkerSize=markersize);
    end
...
xlabel('Unknown Data Threshold (T)');
ylabel('BER');
title(sprintf('N = %d, Oversampling %d, %d QAM, BER', fftSize, OSFactor, modOrder), ...
    Interpreter="latex", FontSize=16);
...
for idxINprob = 1:length(INprobList)
    for idxProcess = startProcess:length(processName)
        ...
        snr_pooleddB = 10 * log10(allData(name).SNR_pooled(idxINprob, :));
        snr_activedB = 10 * log10(allData(name).SNR_active(idxINprob, :));
        plot(ampThresholdList, allData(name).BER(idxINprob, :), '-', ...
            Color=c(idxINprob, :), DisplayName='Actually', ...
            LineWidth=linewidth, MarkerIndices=markerIdx, MarkerSize=markersize);
        plot(ampThresholdList, qamTheoryBERFromPooled(snr_pooleddB, modOrder, fftSize, numData), ...
            '--', Color=c(idxINprob, :), DisplayName='Theory (pooled)', ...
            LineWidth=linewidth, MarkerIndices=markerIdx, MarkerSize=markersize);
        plot(ampThresholdList, qamTheoryBERFromActive(snr_activedB, modOrder), ...
            ':', Color=c(idxINprob, :), DisplayName='Theory (active)', ...
            LineWidth=linewidth, MarkerIndices=markerIdx, MarkerSize=markersize);
...
for idxINprob = 1:length(INprobList)
    for idxProcess = startProcess:length(processName)
        name = processName{idxProcess};
        snrPooled = allData(name).SNR_pooled(idxINprob,:);
        snrActive = allData(name).SNR_active(idxINprob,:);
        betaResult = betaMemorylessTheory(ampThresholdList, INprobList(idxINprob), Scheme='deepclip', ...
                N=fftSize, Los=OSFactor, SNRdB=snr, SINRdB=INsnr, ActiveMask=activeMask, TbFactor=1.4, Mu=2);
        snrCal = snrPooled ./ (1 - betaResult.beta.');
        plot(ampThresholdList, 10 * log10(snrPooled), '-', ...
            Color=c(idxINprob, :), DisplayName=sprintf('p=%s, pooled', string(INprobList(idxINprob))), ...
            LineWidth=linewidth, MarkerIndices=markerIdx, MarkerSize=markersize);
        plot(ampThresholdList, 10 * log10(snrActive), '--x', ...
            Color=c(idxINprob, :), DisplayName=sprintf('p=%s, active', string(INprobList(idxINprob))), ...
            LineWidth=linewidth, MarkerIndices=markerIdx, MarkerSize=markersize);
        plot(ampThresholdList, 10 * log10(snrCal), '-.o', ...
            Color=c(idxINprob, :), DisplayName=sprintf('p=%s, pooled to active', string(INprobList(idxINprob))), ...
            LineWidth=linewidth, MarkerIndices=markerIdx, MarkerSize=markersize);
    end
    % semilogy(ampThresholdList, berPGIR(idxINprob, :), '-', Color=c(idxINprob, :), ...
    %     DisplayName=sprintf("p=%.3f", INprobList(idxINprob)), MarkerIndices=markerIdx, ...
